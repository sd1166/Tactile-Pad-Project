...
def translate_text(text: str):
    ...
    for ch in text:
        ...
        if ch == " ":
            patterns.append({"char": ch, "pattern": BRAILLE_MAP[" "], "type": "space"})
            numeric_mode = False
            continue
        ...
        if is_letter(ch):
            patterns.append(
                {"char": ch, "pattern": BRAILLE_MAP[ch.lower()], "type": "letter"}
            )
            numeric_mode = False
            continue
        ...
        if is_digit(ch):
            ...
            patterns.append(
                {"char": ch, "pattern": DIGIT_TO_BRAILLE[ch], "type": "digit"}
            )
            continue
        ...
        if ch in BRAILLE_MAP:
            patterns.append(
                {"char": ch, "pattern": BRAILLE_MAP[ch], "type": "punctuation"}
            )
        ...
        patterns.append({"char": ch, "pattern": "000000", "type": "unknown"})
        numeric_mode = False
